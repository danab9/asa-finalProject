rule assembly:
    input:
        ...
        long = ((lambda wildcards: samples.at[wildcards.sample, 'long']) if config["skip_trimming"]=='True' and config["decontamination"] != "True" else"results/fastq/decontaminated/{sample}_2.fq" if config["decontamination"] == "True" else "results/fastq/trimmed/{sample}_2_P.fastq.gz") if config["hybrid_assembly"] == "True" else ""
    output:
